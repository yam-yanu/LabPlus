...
if($tanmatu==1){//iphoneなら
 ...
 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '95', '".$_SESSION['name']."', '".$now_time."','','1','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==0){//pcなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '95', '".$_SESSION['name']."', '".$now_time."','','0','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==2){//androidなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '95', '".$_SESSION['name']."', '".$now_time."','','2','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==3){//ガラケーなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '95', '".$_SESSION['name']."', '".$now_time."','','3','".$ipAddress."')";
 $result = executeQuery($sql);
...
$(function(){
			...
			$("#back_kari,#back_l,#back_204,#back_n,#deskb_p,#deskf_p,#back_i").attr("class","hide");//背景削除

			$("#back_s").attr("class","back_g");//さちこの背景表示
		
			$("#img_id_kobatake,#img_id_fujii,#img_id_imairi,#img_id_yamashita,#img_id_yoneshige,#img_id_yamada,#img_id_tamura,#img_id_sakai,#img_id_nomura,#img_id_ishikawa,#img_id_shen").attr("class","hide");
			//人物消去
			...
			$("#yoneshige_b,#yamashita_b,#fujii_b,#tamura_b,#imairi_b,#sakai_b,#nomura_b,#yamada_b,#ishikawa_b,#shen_b").attr("class","hide");

			
		if( <?php echo $ookubo[2]; ?> ==0 || <?php echo $ookubo[2]; ?> ==1){
		...
		if( <?php echo $nobuta[2]; ?> ==0 || <?php echo $nobuta[2]; ?> ==1){
		...
		if( <?php echo $tsujii[2]; ?> ==0 || <?php echo $tsujii[2]; ?> ==1){
		...
		if( <?php echo $yano[2]; ?> ==0  ||  <?php echo $yano[2]; ?> ==1){
		...
		if( <?php echo $muramoto[2]; ?> ==0 || <?php echo $muramoto[2]; ?> ==1){
		...
		if( <?php echo $kotera[2]; ?> ==0 || <?php echo $kotera[2]; ?> ==1){
		...
		if( <?php echo $kitahara[2]; ?> ==0 || <?php echo $kitahara[2]; ?> ==1){
		...
		if( <?php echo $watanabe[2]; ?> ==0 ||  <?php echo $watanabe[2]; ?> ==1){
 ...
 function hyou(no) {
	...
	document.location = "http://49.212.200.39/login/kitahara/phpverpre.php";
	...
	if(no==9){document.location = "http://49.212.200.39/login/kitahara/phpver.php";}//コメントのページへ

...
if($tanmatu==1){//iphoneなら
 ...
 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '94', '".$_SESSION['name']."', '".$now_time."','','1','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==0){//pcなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '94', '".$_SESSION['name']."', '".$now_time."','','0','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==2){//androidなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '94', '".$_SESSION['name']."', '".$now_time."','','2','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==3){//ガラケーなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '94', '".$_SESSION['name']."', '".$now_time."','','3','".$ipAddress."')";
 $result = executeQuery($sql);
...
$(function(){
			...
			$("#back_kari,#back_204,#back_l,#back_s,#back_n,#deskb_p,#deskf_p").attr("class","hide");//背景を消去


			$("#img_id_fujii,#img_id_nobuta,#img_id_tsujii,#img_id_yano,#img_id_muramoto,#img_id_kotera,#img_id_imairi,#img_id_kitahara,#img_id_watanabe,#img_id_sakai,#img_id_nomura,#img_id_shen").attr("class","hide");
			//人物消去
			...
			$("#kotera_b,#nobuta_b,#fujii_b,#imairi_b,#sakai_b,#nomura_b,#watanabe_b,#kitahara_b").attr("class","hide");
			//ステータス消去

			$("#back_i").attr("class","back_g");//ITSKY3の背景表示
			document.bgColor = "#000000";



		if( <?php echo $kobatake[2]; ?> ==0 ||  <?php echo $kobatake[2]; ?> ==1){
		...
		if( <?php echo $yamashita[2]; ?> ==0 ||  <?php echo $yamashita[2]; ?> ==1){
		...
		if( <?php echo $ishikawa[2]; ?> ==0  ||  <?php echo $ishikawa[2]; ?> ==1){
		...
		if( <?php echo $yoneshige[2]; ?> ==0 ||   <?php echo $yoneshige[2]; ?> ==1){
		...
		if( <?php echo $yamada[2]; ?> ==0 ||  <?php echo $yamada[2]; ?> ==1){
		...
		if( <?php echo $tamura[2]; ?> ==0 ||  <?php echo $tamura[2]; ?> ==1){
...
<input type="image" onClick="hyou(2);" width="90" height="90" src="./img/topbutton.png">

...
if($tanmatu==1){//iphoneなら
 ...
 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '93', '".$_SESSION['name']."', '".$now_time."','','1','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==0){//pcなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '93', '".$_SESSION['name']."', '".$now_time."','','0','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==2){//androidなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '93', '".$_SESSION['name']."', '".$now_time."','','2','".$ipAddress."')";
 $result = executeQuery($sql);


}else if($tanmatu==3){//ガラケーなら

 $sql = "INSERT INTO kitahara VALUES('".$_SESSION['team_id']."', '".$_SESSION['id']."', '93', '".$_SESSION['name']."', '".$now_time."','','3','".$ipAddress."')";
 $result = executeQuery($sql);
...
$(function(){
				...
				$("#back_kari,#back_204,#back_l,#back_s,#back_i,#deskb_p,#deskf_p").attr("class","hide");//仮TOPの背景を消して

			$("#img_id_kobatake,#img_id_nobuta,#img_id_tsujii,#img_id_yano").attr("class","hide");
			$("#img_id_muramoto,#img_id_kotera,#img_id_yamashita,#img_id_ishikawa").attr("class","hide");
			$("#img_id_kitahara,#img_id_yoneshige,#img_id_yamada,#img_id_tamura,#img_id_watanabe").attr("class","hide");
			...
			$("#yoneshige_b,#yamashita_b,#tamura_b,#nobuta_b,#watanabe_b,#kotera_b,#yamada_b,#ishikawa_b,#kitahara_b").attr("class","hide");
			//ステータス、人物、背景の消去

			$("#back_n").attr("class","back_g");//NIS-Xの背景表示
			document.bgColor = "#000000";

		if(<?php echo $imairi[2]; ?> ==0 || <?php echo $imairi[2]; ?> ==1){
		...
		if(<?php echo $fujii[2]; ?> ==0 ||  <?php echo $fujii[2]; ?> ==1){
		...
		if(<?php echo $sakai[2]; ?> ==0  ||  <?php echo $sakai[2]; ?> ==1){
		...
		if(<?php echo $nomura[2]; ?> ==0 ||  <?php echo $nomura[2]; ?> ==1){
		...
		if(<?php echo $shen[2]; ?> ==0 ||  <?php echo $shen[2]; ?> ==1){

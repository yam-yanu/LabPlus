<?php
  ...
  $sql = "SELECT login.IDNo,login.ID,login.LOGIN_NAME,login.LOCAL_P,login.COMMON_P,login.RELATION_P,login.RANKING,login.GOLD,login.TEAM_ID,team.COLOR
	FROM login INNER JOIN team ON login.TEAM_ID=team.ID WHERE login.TEAM_ID != 0";
  ...
  if($rows){
	...
	while($row = mysql_fetch_array($result)){
		...
		//得た称号の優先度が高いものを表示
		$sql_degree = "SELECT degree.MASTERNAME from get_degree INNER JOIN degree ON get_degree.DEGREEID = degree.ID WHERE get_degree.OBTAINID = ".$row["IDNo"]." LIMIT 1";
		$result_degree = executeQuery($sql_degree);
		$rows_degree = mysql_num_rows($result_degree);
		if($rows_degree){
			$row_degree = mysql_fetch_array($result_degree);
			$degree = $row_degree["MASTERNAME"];
		}else{
			$degree = "なにも得ぬ者";
		}
...
<script type="text/javascript">
	...
	function check(){
		if(document.make_praise_form.praise.value == "" || document.make_praise_form.degree.value == ""){
...
<body>
  <div id="wrap">
		...
		<div id="inner">
			...
			<div id="centerWrap">
				<div class="title">ほめる部分の作成</div>
				<form name="make_praise_form" action="complete_make.php" method="post">
					<table>
						<tr><td>ほめる部分(20文字まで)</td><td><input type="text" name="praise" maxlength="20"></td></tr>
						<tr><td>マスターしたときの称号<br>(20文字まで)</td><td><input type="text" name="degree" maxlength="20"></td></tr>
					</table>
					<input type="submit" name="submit" value="作成" onclick="return check()">
				</form>
				<?= $msg ?>
				<?= $my_member ?>
